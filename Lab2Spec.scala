...
class Lab2Spec(lab2: Lab2Like) extends FlatSpec {
  ...
  it should "return false if two number values are not the same" in {
    val e1 = S("AB")
    val e2 = S("B")
    val e3 = eval(Binary(Lt, e1, e2))
    assert(e3 === B(true))
  ...
  it should "return false if two string values are the same" in {
    val e1 = N(5)
    val e2 = N(5)
    val e3 = eval(Binary(Lt, e1, e2))
    assert(e3 === B(false))
  ...
  it should "eval to N(8)" in {
    val e1 = N(1)
    val e2 = Binary(Plus, Var("x"), N(3))
    val e3 = eval(Binary(Plus, ConstDecl("x", e1, e2), N(4)))
    assert(e3 === N(8))
  }
